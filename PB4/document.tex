...
\emph{Object} erwartet. Ist der Typ jedoch durch
...
solcher Fall implementiert\footnote{vgl. Abbildung 3 - TestTable-GUI}.
Bezüglich der Wartbarkeit und der Objektorientierung sollte bei Angabe eines
falschen Types nicht einfach der Standardwert zurückgegeben werden. Vielmehr
sollte hier darauf hingewiesen werden, dass der Falsche Typ verwendet wurde.
Dies kann zum einen im Code durch eine generische Erweiterung der
Vaadin-\emph{Table} oder durch entsprechende Fehlermeldungen realisiert
werden.\\
Bei Profil C/S werden nicht direkt die Objekte in die Tabellen geladen, sonder
Tabellen-Daten-Objekte, die ihre Werte wiederrum von den entsprechenden
Buinessobjekten beziehen. Dadruch wurde zwischen Visualisierung und Daten eine
weitere Schicht eingezogen, die dem Multichannel zugute kommt. Würde diese
Schicht nicht vorliegen, müsste die Übernahme der Werte aus den Businessobjekten
in die Tabellen doppelt implementiert werden.\\
Im Prototypen kann somit über das Inteface \emph{TableData} direkt auf die
Werte, der Businessobjekte zugegriffen werden. Egal in welchem Client man sich
befindet, werden immer \emph{SimpleStringProperties} zurückgegeben, die vom
Multichannel aus als Strings weitergegeben werden und an der
entsprechenden Stellen ausgegeben werden können.\\
...
orientiert.\\
Weitere Gemeinsamkeiten und Unterschiede sind Abbildung 2 zu entnehmen.
...
\hline
	...
	\begin{figure}[!htb]
			...
			\caption{Abbildung 3 - TestTable-GUI}
		\end{figure}
	\begin{figure}[!htb]
