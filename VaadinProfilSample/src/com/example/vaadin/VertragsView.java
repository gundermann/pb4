...
class VertragsView extends BorderLayout {
	...
	private void initController() {
		controlbuttons.get("first").addClickListener(new Button.ClickListener() {

                    @Override
                    public void buttonClick(Button.ClickEvent event) {
                        currentTV.changeState(controlbuttons.get("first").getCaption());
				setupOrUpdateStateGraph();
                    }
                });
	...
	private void changeStateID(String state){
		...
		for (int j = 0; j < grid.getRows(); j++){
			...
			if(label.getCaption() == null || label.getCaption().equals(state)){   //getText()   auf NULL prüfen
